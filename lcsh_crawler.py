...
Usage
-----
    python3 lcsh_crawler.py [options]

Options
-------
    --start-url URL     Feed page URL to begin crawling.
                        Default: https://id.loc.gov/authorities/subjects/activitystreams/feed/1
...
--------
    ...
    python3 lcsh_crawler.py

    # Start from the most recent page and work backward
    python3 lcsh_crawler.py \\
        --start-url https://id.loc.gov/authorities/subjects/feed/123439 \\
        --direction backward

    # Dry run to preview matches
    python3 lcsh_crawler.py --dry-run
"""
...
def safe_filename(url: str, today_str: str) -> str:
    """Convert a feed URL to a filename with the LCSH prefix, feed number, and date."""
    # e.g. http://id.loc.gov/authorities/subjects/activitystreams/feed/97
    #   -> LCSH_Activity_Stream_97_2026-06-09.json
    m = re.search(r"/feed/(\d+)$", url)
    feed_num = m.group(1) if m else re.sub(r"[^\w]", "_", url)[-20:]
    return f"LCSH_Activity_Stream_{feed_num}_{today_str}.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Save LCSH feed pages that contain items published today.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--start-url",
        default="http://id.loc.gov/authorities/subjects/activitystreams/feed/1",
